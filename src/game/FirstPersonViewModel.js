...
export default function FirstPersonViewModel({ actionAnimationRef, enabled = true }) {
  ...
  const basePosition = useMemo(() => new THREE.Vector3(0.66, -0.08, -0.76), []);
  ...
  useFrame(({ clock }) => {
    ...
    group.rotation.set(-0.04 - swing * 0.8, 0.12 + swing * 0.44, -0.42 - roll);

    if (handRef.current) handRef.current.rotation.z = -0.2 + swing * 0.3;
    if (itemRef.current) {

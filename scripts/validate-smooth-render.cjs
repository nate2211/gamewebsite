...
expect(renderer.includes('!centerChunkMounted && <LoadingVoxelPrimer'), 'Fallback voxel ground must remain until the player center chunk is mounted.');
...
expect(streamer.includes('queueRef.current.length ? 36 : 90'), 'Chunk applications must be paced to avoid render spikes.');
